...
waitForCanvas((canvas) => {
  ...
  function playNext() {
    ...
    playing = true;
    const gloss = queue[0]; // Peek at next
    ...
    } else if (pendingLoads[gloss] === 'failed') {
      queue.shift(); // consume
      // Placeholder arm wave when GLB anim not loaded/missing
    ...
    } else if (pendingLoads[gloss] === 'loading') {
      setTimeout(playNext, 100);
    } else {
      // Try to dynamically load it
      pendingLoads[gloss] = 'loading';
      ...
      animLoader.load(
        ...
        (gltf) => {
          if (mixer && gltf.animations.length) {
            ...
            action.loop = THREE.LoopOnce;
            action.clampWhenFinished = true;
            actions[gloss] = action;
            pendingLoads[gloss] = 'loaded';
            console.log('[Echo-Sign] Dynamically loaded:', gloss);
          } else {
            pendingLoads[gloss] = 'failed';
          }
          playNext(); // retry
        },
        undefined,
        (err) => {
          console.warn(`[Echo-Sign] You need to download ${gloss}.glb into assets/animations/`);
          pendingLoads[gloss] = 'failed';
          playNext(); // retry (will hit fallback)
        }
      );
    }
  }
